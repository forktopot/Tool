...
class SstvInfoTab(IMessageEditorTab):
    ...
    def isEnabled(self, content, isRequest, headers):  # only show tab in response
    ...
    def setMessage(self, content, isRequest, headers):
        if content:
            pretty_msg = ''
        ...
        if headers:
            pretty_msg = ''
            shiro = stringIsShiro(headers)
            if shiro != False:
                pretty_msg += "Find shiro:" + shiro + '\n'
...
def stringIsShiro(string):
    Shiros = re.findall(r'(rememberMe=deleteMe)', string)
    if Shiros != []:
        Shiros = set(Shiros)
        Shiros = ','.join(Shiros)
        return Shiros
